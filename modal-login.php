...
if (isset($_GET['modal'])) {
    ...
    echo '<div style="padding: 20px; text-align: center;">
            ...
            <a href="modal-login.php?modal=1" class="btn btn-primary">Go Back to Login</a>
            <script>
                // Tell parent to keep login tab active
                if (window.parent) {
                    window.parent.postMessage({
                        action: "keepLoginTabActive"
                    }, "*");
                }
            </script>
          </div>';
    exit();
        ...
        if (detectSuspiciousLogin($user['id'])) {
            ...
            if (sendEmail($email, $subject, $body)) {
                ...
                echo '<div class="alert alert-info mb-3">Verification code sent to your email</div>
                      <script>
                        // Show login tab
                        const loginTab = window.parent.document.querySelector("#login-tab");
                        const loginPane = window.parent.document.querySelector("#login-tab-pane");
                        if (loginTab && loginPane) {
                            new window.parent.bootstrap.Tab(loginTab).show();
                            loginPane.classList.add("show", "active");
                            
                            const signupTab = window.parent.document.querySelector("#signup-tab");
                            const signupPane = window.parent.document.querySelector("#signup-tab-pane");
                            if (signupTab && signupPane) {
        ...
        echo '<script>
                ...
                const loginModal = window.parent.bootstrap.Modal.getInstance(
                    window.parent.document.getElementById("loginRequiredModal")
                );
                if (loginModal) {
                    loginModal.hide();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['request_reset'])) {
        ...
        if (empty($email)) {
            header("Location: modal-login.php?reset_error=Please enter your email address.");
            exit();
        } else {
            ...
            if ($result->num_rows === 1) {
                ...
                if ($updateStmt->execute()) {
                    ...
                    if ($emailSent) {
                        ...
                        header("Location: modal-login.php?reset_success=1");
                        exit();
                    } else {
                        header("Location: modal-login.php?reset_error=Failed to send email after multiple attempts. Please try again later.");
                        exit();
                    }
                } else {
                    header("Location: modal-login.php?reset_error=Database error. Please try again.");
                    exit();
                }
                $updateStmt->close();
            } else {
      ...
      <form method="post" id="loginForm">
        <div class="row g-3">
          ...
          <div class="col-12 text-center">
            <p class="mt-3">Don't have an account? <a href="#" onclick="switchToSignupTab()">Sign up</a></p>
          </div>
        </div>
      </form>
    </div>
...
<?php if (isset($_GET['modal'])): ?>
        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                const res = await fetch('modal-login.php?modal=1', {
                    method: 'POST',
                    body: formData
                });
                const data = await res.json();
                if (data.success) {
                    window.parent.postMessage(JSON.stringify({loginSuccess: true}), window.location.origin);
                } else if (data.needVerification) {
                    window.parent.postMessage(JSON.stringify({showMessage: data.message}), window.location.origin);
                    // Replace login form with verification form in iframe
                    const container = form.parentElement;
                    container.innerHTML = `
                        <p>We've sent a 6-digit verification code to your email.</p>
                        <form id="verifyForm">
                            <div class="mb-3">
                                <input type="text" name="verification_code" class="form-control" placeholder="6-digit code" required maxlength="6" pattern="\\d{6}">
                            </div>
                            <button type="submit" class="btn btn-primary">Verify</button>
                        </form>
                    `;
                    const verifyForm = document.getElementById('verifyForm');
                    verifyForm.addEventListener('submit', async function(ve) {
                        ve.preventDefault();
                        const vFormData = new FormData(verifyForm);
                        vFormData.append('verify_login', '1');
                        const vRes = await fetch('modal-login.php?modal=1', {
                            method: 'POST',
                            body: vFormData
                        });
                        ...
                        if (vData.success) {
                            ...
                            window.parent.postMessage(JSON.stringify({showMessage: vData.message}), window.location.origin);
                        }
                    });
                } else {
                    window.parent.postMessage(JSON.stringify({showMessage: data.message}), window.location.origin);
                ...
                window.parent.postMessage(JSON.stringify({showMessage: 'An error occurred. Please try again.'}), window.location.origin);
            }
        });
        <?php endif; ?>
    });
    ...
    if (window.parent) {
        // Switch to signup tab in parent window
        window.parent.postMessage({
            action: "switchToSignupTab"
        }, window.parent.location.origin);
    }
}
</script>
