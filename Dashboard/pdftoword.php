...
try {
    ...
    $stmt = $db->prepare('SELECT * FROM conversions 
                         ...
                         AND conversion_type = "pdf_to_word" 
                         AND status != "failed" 
...
<head>
    ...
    <title>PDFSimba | PDF to Word Conversions</title>
    <!-- Bootstrap CSS -->
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#pdfToWordModal">
    ...
    <?php if (empty($documents)): ?>
        <div class="alert alert-info">
            <i class="bi bi-info-circle"></i> No converted Word documents available.
        </div>
    <?php else: ?>
                ...
                <div class="col">
                    <div class="card document-card">
                        <div class="card-body text-center">
                            <i class="bi bi-file-earmark-word-fill document-icon mb-3"></i>
                            ...
                            <div class="d-grid gap-2">
                                <!-- Download link -->
                                <a href="<?= BACKEND_URL ?>/converted/<?= htmlspecialchars($doc['converted_filename']) ?>" 
                                   class="btn btn-sm btn-outline-primary" download>
                                    <i class="bi bi-download"></i> Download Word
                                </a>
                                <!-- Edit link -->
                                <a href="/dashapi/editor/<?= str_replace('.docx', '.html', htmlspecialchars($doc['converted_filename'])) ?>?conversion_id=<?= $doc['conversion_id'] ?>&user_id=<?= $user_id ?>" 
      ...
      <form id="pdfToWordForm" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div id="pdfToWordLoading" style="display: none;">
            ...
            <span class="ms-2">Converting PDF to Word...</span>
          </div>
          <div id="pdfToWordResult" style="display: none;">
            <div class="alert alert-success">
              Conversion successful! The document will appear in your list above.
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
          <button type="submit" class="btn btn-primary">Convert to Word</button>
        </div>
      </form>
    </div>
  </div>
...
// PDF to Word Conversion Form
const pdfToWordForm = document.getElementById("pdfToWordForm");
if (pdfToWordForm) {
    pdfToWordForm.addEventListener("submit", function(e) {
        e.preventDefault();

        const form = e.target;
        const formData = new FormData(form);
        const submitButton = form.querySelector('button[type="submit"]');
        const loadingDiv = document.getElementById("pdfToWordLoading");
        ...
        .then((data) => {
            ...
            setTimeout(() => {
                bootstrap.Modal.getInstance(document.getElementById('pdfToWordModal')).hide();
