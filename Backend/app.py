...
# Word to PDF
import word_to_pdf as w2p

@app.route('/convert-word-to-pdf', methods=['POST'])
def convert_word_route():
    print("DEBUG convert_word_file from:", w2p.__file__, flush=True)
    print("DEBUG function object:", w2p.convert_word_file, flush=True)
    file = request.files.get('word_file')
    return w2p.convert_word_file(file)
...
def download_protected(filename):
    try:
        ...
        return send_from_directory(
            ...
            filename,
            as_attachment=True,
            mimetype='application/pdf',  # Explicitly set MIME type
