...
def convert_excel_to_pdf():
    try:
        ...
        # Conversion command with additional parameters for better Excel conversion
        cmd = [
            LIBREOFFICE_PATH,
            '--headless',
        ...
        try:
            result = subprocess.run(
                ...
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                timeout=60  # Give more time for Excel conversions
            )
...
def convert_ppt_to_pdf():
    try:
        ...
        # Conversion command with specific PowerPoint export filter
        cmd = [
            LIBREOFFICE_PATH,
            '--headless',
        ...
        try:
            result = subprocess.run(
                ...
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                timeout=60  # PowerPoint conversions might take longer
            )
...
<script>
// Configuration
const pdfUrl = "/api/uploads/{{ document_id }}";
...
async function saveSignedDocument() {
  ...
  try {
    ...
    const response = await fetch("/api/save-signed-document", {
      method: "POST",
      headers: { 
...
cleanup_old_files([UPLOAD_FOLDER, OUTPUT_FOLDER], max_age_minutes=120, check_interval_seconds=7200)
